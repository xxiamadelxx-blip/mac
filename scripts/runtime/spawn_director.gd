...
func phase_for(simulation_seconds: float, boss_kind: String = "") -> String:
    ...
    var elapsed: float = clamp(simulation_seconds - start_seconds, 0.0, cycle_duration)
    ...
    var recovery_end: float = min(cycle_duration, suppression_seconds + recovery_seconds)
    var siege_start: float = max(0.0, cycle_duration - siege_seconds)
