...
| Есть run-clock и encounter-clock | Architecture runtime contract | before/after values на RUN_ACTIVE, pause и boss freeze | R1 |
...
| B1 wave bands data-driven | B1 wave table/model | trace на границах 2/5/10/15/20 минут, budget/cap evidence | R2 |
| Spawn соблюдает active cap | B1 acceptance | occupancy trace и controlled fallback при переполнении | R2 |
| Boss spawn безопасен и читаем | Architecture/B1 | boss encounter ID, safe distance, telegraph/reaction evidence | R2 |
| Время забега останавливается на каждом боссе | Решение владельца проекта; RUNTIME_CONTEXT | trace для каждого boss boundary: run time unchanged during boss phases | R2 |
| Wave/XP/spawn clocks заморожены на боссе | Runtime context | no spawn/XP/wave progression during BOSS_INTRO/BOSS_ACTIVE/settlement | R2 |
| Боссовый encounter clock работает отдельно | Runtime context | encounter time progresses only in BOSS_ACTIVE | R2 |
...
| Нефинальный boss chest отдельный | Architecture contract | chest_offer_id, eligibility/fallback, claim idempotency | R2 |
| Final boss не создаёт boss chest | State machine and data contract | final trace has final settlement → victory and no chest_offer | R2 |
...
6. На любом боссе run-clock и обычные wave/XP/spawn clocks не продвигаются.
7. Финальный босс не открывает boss chest.
