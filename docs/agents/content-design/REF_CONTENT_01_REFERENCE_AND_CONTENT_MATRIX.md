...
Parent HEAD: `6d7c2a361944ada398622e77c2436a79a47baf46`
...
6. Для каждой синергии зафиксирован gate: оружие уровня 10 + связанная общая пассивка ранга 10; вместимость билда остаётся 6/6, забег — 1800 секунд, общий предел — 5 синергий.
...
Общее условие эволюции: оружие достигло 10-го уровня, пассивка достигла 10-го ранга, оружие ещё не эволюционировано, пара совпадает по `synergy_id`, источник — нефинальный `BOSS_CHEST`, `encounter_kind` равен `MAIN_BOSS` или `MINI_BOSS`, а число подтверждённых синергий в забеге меньше 5. Десять — это максимум развития компонентов, а не число слотов; вместимость билда остаётся 6/6. Числовой вклад и кривую рангов закрепляет агент баланса.
...
Контракт этого среза содержит ровно 10 обычных/base enemy ID. Восемь записей защищены Stage 04, а `enemy_lotus_usher` и `enemy_moonroot_burrower` уже существуют как предложенные MAC ID в content index и здесь закрепляются как две дополнительные обычные записи со статусом `PROPOSED / REGISTRY_SYNC_PENDING`. Две старые Stage 04 записи — `enemy_stone_oni` и `enemy_eclipse_serpent` — вынесены в отдельный legacy-слой `ELITE_FAMILY` и не входят в счёт обычных десяти. Ни одна запись не становится мини-боссом или главным боссом, а тихая переклассификация запрещена.
...
| `enemy_lotus_usher` — Лотосовый зазыватель | Отмечает короткие цветковые сектора, которые закрываются по очереди | `zone`, `controller`, `telegraph`, `route` | 10–15 мин; proposed ordinary extension | выйти из первого сектора и вернуться после импульса | `ZoneStore`, `TelegraphResolver`, `MovementSystem` | `EXISTING_CONTENT_PROPOSED / REGISTRY_SYNC_PENDING` |
| `enemy_moonroot_burrower` — Луннокорневой землерой | Уходит под землю и возвращается в отмеченной точке, меняя безопасную линию | `burrow`, `ambush`, `reposition`, `telegraph` | 15–20 мин; proposed ordinary extension | покинуть маркер и атаковать после восстановления выхода | `EncounterState`, `TelegraphResolver`, `MovementSystem`, `CombatSystem` | `EXISTING_CONTENT_PROPOSED / REGISTRY_SYNC_PENDING` |

### Две существующие ELITE_FAMILY записи вне десятки обычных

Эти две записи сохраняются для совместимости с Stage 04 и не удаляются. Они не входят в целевой счёт 10 обычных и не считаются двумя из десяти новых элитных вариаций.

| ID | Роль и подпись поведения | Теги | Первая точка появления | Контр-решение игрока | Зависимости | Статус |
|---|---|---|---|---|---|---|
| `enemy_stone_oni` — Каменный они | Усиленный удар по земле с длинным предупреждением и восстановлением | `elite`, `slam`, `zone`, `heavy` | 10–15 мин; legacy Stage 04 | покинуть крупный телеграф и атаковать восстановление | `EliteDirector`, `TelegraphResolver`, `ZoneStore`, `XPDrop` | `EXISTING_CONTENT_SPECIFIED / ELITE_FAMILY / LEGACY_NOT_IN_ORDINARY_COUNT` |
| `enemy_eclipse_serpent` — Змей затмения | Усиленный рывок по дуге с оставляемым следом | `elite`, `dash`, `trail`, `arc` | 15–20 мин; legacy Stage 04 | пересечь путь после поворота и не входить в след | `EliteDirector`, `TelegraphResolver`, `ZoneStore`, `XPDrop` | `EXISTING_CONTENT_SPECIFIED / ELITE_FAMILY / LEGACY_NOT_IN_ORDINARY_COUNT` |

Точные HP, урон, скорость, веса состава, активный лимит, ступень XP, элитный ритм и продолжение после 30 минут принадлежат агенту баланса и `ContentRegistry`; здесь они не задаются.

## 6A. Десять предложенных элитных вариаций

Эти десять записей — отдельные варианты поверх десяти обычных/base записей, по одному варианту на каждую обычную семью. Они не заменяют базовые ID, не становятся мини-боссами, не добавляются в постоянный roster и не дают автоматический артефакт. Все ID ниже впервые вводятся в этой ревизии и имеют только статус `PROPOSED`; их числовые параметры и точные окна появления принадлежат B1.
...
| `elite_variant_lotus_usher_bloomguard` — Цветковый страж лотосового зазывателя | `enemy_lotus_usher` | Два сектора открываются как ложный безопасный маршрут, затем один закрывается с задержкой; сменить линию до второго сигнала | `elite`, `zone`, `feint`, `route`, `recovery` | полоса базовой записи; точное elite-окно `PENDING_BALANCE` | `EliteDirector`, `ZoneStore`, `TelegraphResolver`, `MovementSystem` | `PROPOSED / PENDING_BALANCE / PENDING_ARCHITECTURE` |
| `elite_variant_moonroot_burrower_gravewake` — Землерой могильного следа | `enemy_moonroot_burrower` | Подземный маршрут раздваивается, а после выхода остаётся задержанный след; читать маркер выхода и атаковать восстановление | `elite`, `burrow`, `ambush`, `trail`, `recovery` | полоса базовой записи; точное elite-окно `PENDING_BALANCE` | `EliteDirector`, `EncounterState`, `TelegraphResolver`, `MovementSystem` | `PROPOSED / PENDING_BALANCE / PENDING_ARCHITECTURE` |
...
| Десять обычных записей плюс десять элитных вариаций | 8 Stage 04 записей + 2 уже предложенных обычных MAC ID дают ровно 10 ordinary; 2 legacy `ELITE_FAMILY` записи исключены из ordinary-счёта, а 10 вариантов описаны отдельными `PROPOSED` IDs | `PENDING_ARCHITECTURE / REGISTRY_SYNC_PENDING` |
...
- Матрица содержит 10 связок оружие-пассивка-синергия, 10 общих пассивок забега, 10 артефактов, 10 обычных записей противников, 10 отдельных элитных вариаций и 4 базовых босса; 2 legacy `ELITE_FAMILY` записи сохранены вне целевого счёта.
...
3. Агент баланса должен закрепить одну числовую таблицу для десяти оружий, десяти пассивок, десяти артефактов, десяти обычных противников, десяти элитных вариаций, legacy-пула Stage 04, боссов, волн, сундуков, резервных результатов и дерева магазина.
4. Architecture/Product должны сопоставить 10 обычных + 10 элитных вариаций с реестром: текущий sync-контракт допускает максимум 5 зарегистрированных элитных вариантов, поэтому новые ID остаются `PROPOSED` до решения.
5. Две legacy `ELITE_FAMILY` записи Stage 04 требуют явного compatibility mapping и не должны автоматически попасть в ordinary roster.
