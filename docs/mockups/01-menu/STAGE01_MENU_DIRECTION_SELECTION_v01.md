...
## Русская слоёная итерация

Для экранов Direction B добавлены слои в папке `layers/`:

- исходный ART оставлен без перегенерации;
- UI_RU содержит непрозрачные маски старых подписей и точный русский текст;
- COMPOSITE_RU используется для визуальной проверки результата.

Отдельная попытка перегенерации чистого фона отклонена: она изменила лицо, причёску и костюм героини. В репозиторий она не добавлялась.
