...
Навыки нужно прочитать полностью до использования. Если конкретный skill недоступен, зафиксируй это в отчёте и используй безопасный fallback. Code-work не нужен: в этой задаче запрещена реализация runtime. Visual Lab нужен только если ты решишь менять visual/art assets, а это запрещено.
