...
REPO_CONTEXT.md — исторический baseline, а не live HEAD. Перед началом самостоятельно проверь текущие repository, branch и HEAD. Расхождение live HEAD с этим baseline не является ошибкой и не требует переписывать context packet.

Твоя задача — не написать общий список идей и не сделать вид, что runtime уже существует. Твоя задача — на основании текущего репозитория спроектировать логическую архитектуру первого 30-минутного забега так, чтобы следующий coding-агент мог реализовать её по проверяемому контракту.
...
- шесть основных боссов на target-базе 5, 10, 15, 20, 25 и 30 минут; два mini-boss из C3 и один новый промежуточный slot;
...
- chest-window registry до 15 окон: 8 нефинальных boss/mini-boss windows и 7 резервных source windows; финальный boss chest запрещён;
- wave cycles с post-boss relief → gradual ramp → pre-boss peak;
- три data-driven варианта enemy_ink_beetle и три новых enemy slots без выдуманных чисел/имён;
