...
Первым создаётся набор из трёх направлений меню в `docs/mockups/01-menu/`. Направление B рекомендовано как база; screen-flow пакет Home/Heroes/Run Setup/Loading/Victory/Defeat добавлен туда же. Последующие визуальные этапы не считаются начатыми до закрытия этапа 1.